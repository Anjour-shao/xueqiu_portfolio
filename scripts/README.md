...
**每日 Digest**（云端 GHA、预览、触发脚本）已移至 [`daily_digest/`](../daily_digest/README.md)。

已删除的冗余脚本：`xueqiu_api_probe.py`、`sina_hfq_cli.py`、`recompute_all.py`、`test_xueqiu_api.py`、`probe_cube_sources.py`、`sync_benchmark.py`（基准请用 `sync_tushare_aux.py --benchmark-only`）。
