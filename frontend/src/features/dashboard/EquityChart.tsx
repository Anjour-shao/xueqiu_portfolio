...
function buildKlineOption(series: ChartSeries, hasIndex: boolean, zoomStart: number): EChartsOption {
...
export function EquityChart({
  ...
  const option = useMemo(() => {
    if (!series) return {};
    return buildKlineOption(series, hasIndex, zoomStart);
  }, [series, hasIndex, zoomStart]);
  ...
  const onEvents = useMemo(
    ...
    [onBrushEnd, onUpdateAxisPointer, onGlobalOut],
  );

  const hoverTooltip =
    hoverIndex != null && tooltipPos != null && series ? (() => {
      ...
      return (
        <EquityChartDayTooltip
          ...
          top={tooltipPos.top}
          onPointerEnter={lockTooltip}
          onPointerLeave={unlockTooltip}
        />
      );
    })() : null;
  ...
  const toolbarHeight = showBrushControls ? 34 : 0;
    ...
    >
      {showBrushControls && (
        <Stack
          ...
          flexWrap="wrap"
          sx={{ mb: 0.5, minHeight: toolbarHeight, gap: 1 }}
        >
